...
估算函数是故意"粗糙"的。精确的 tokenizer 调用很慢，对于压缩决策来说，4 字符/token 的近似已经足够。Claude Code 也用类似的快速估算。值得注意的是，cli.js 在消息级别（Wp1）的 token 计数中还会额外乘以 1.333x 系数作为安全余量，这里为了教学清晰而省略了这个乘数。
