...
| 側面 | 本番 cli.js | 教育版 |
|------|-----------|--------|
| アイドル機構 | パッシブなturnレベルアイドル（入力注入待ち） | アクティブポーリング（1秒sleep、受信箱/ボード確認） |
| 自動割当 | プロンプト駆動（モデルがTaskList + TaskUpdateを呼ぶ） | コード駆動（idleループ内の `_scan_unclaimed_tasks()`） |
| アイデンティティ注入 | システムattachmentパイプライン経由 | `auto_compact` 後に明示的に再注入 |

教育版はアクティブポーリングを使い、アイドル/ウェイクアップパターンを観察・デバッグ可能にしている。本番環境はturnレベルイベント注入で同等の効果を達成する。
