...
| 方面 | 生产 cli.js | 教学版本 |
|------|-----------|---------|
| 空闲机制 | 被动的 turn 级空闲（等待输入注入） | 主动轮询循环（sleep 1s，检查收件箱/看板） |
| 自动认领 | 提示词驱动（模型调用 TaskList + TaskUpdate） | 代码驱动（idle 循环中的 `_scan_unclaimed_tasks()`） |
| 身份注入 | 通过系统 attachment pipeline 处理 | `auto_compact` 后显式重新注入 |

教学版本使用主动轮询来让空闲/唤醒模式可观察、可调试。生产环境通过 turn 级事件注入达到相同效果。
