...
关键：只清理**内容**，保留工具调用的结构。模型仍然知道它调用过什么，只是看不到旧输出了。仅当预估节省量 >= MIN_SAVINGS（20000 tokens，匹配 cli.js zUY=20000）时才执行清理。
...
当上下文超过动态阈值时触发。生产 cli.js 的 auto_compact 替换整个消息列表，没有"保留最近 N 条"的行为：
